    ...
    class SitemapIndexUrl < SitemapUrl
      ...
      def initialize(path, options={})
        if index = path.is_a?(SitemapGenerator::Builder::SitemapIndexFile) && path
          options = SitemapGenerator::Utilities.reverse_merge(options, :host => index.location.host, :lastmod => Time.now, :changefreq => SitemapGenerator.default_changefreq, :priority => 1.0)
